...
**Última atualização:** 2026-08-27. **Fase atual:** v3.0 em uso real; ciclo **v3.1 — Faixa de normalidade e gráfico interativo** entregue, incluindo três defeitos de uso real relatados pelo usuário e corrigidos (seções 8.6 e 8.7). Q1 a Q31 fechadas, ADRs `0001` a `0015`, migrações `001` a `009`. A Faixa de normalidade **está implementada e semeada**: 78 faixas, 55 Indicadores, fonte citada em cada linha. **Em aberto: Q32 (seção 3), banda de faixa de um lado só, a prova de imagem no UNRAID (8.5), e o deploy dos três fixes de hoje na instância de produção (8.7).**
...
**Onde o trabalho está:** branch `main`, working tree limpo, tudo commitado e sincronizado. Três commits cobrem a v3.1: `c6d8bad` (grill Q18–Q31, ADRs `0012`–`0015`, migração `008`, `/config`, Chart.js), `50a02a2` (dois defeitos de `/config` corrigidos, resolução da Faixa de normalidade) e `d784ec8` (migração `009` com as 78 faixas, limites em destaque na tela). Sessão de 2026-08-27 acrescenta três commits de correção de defeito real, ver 8.6 e 8.7: `f9664d6` (chave duplicada no `{#each}` da tela Indicadores), `eb482e3` (`unmapped:null` na tela de Revisão) e `8ff60f3` (chave duplicada num segundo `{#each}`, agora na lista de pendências da tela de Revisão). **Nenhum dos três está em produção ainda** — ver 8.7.
...
### 8.7 Sessão 2026-08-27 (continuação): terceiro defeito, e a produção estava sem nenhum dos três fixes

Usuário relatou uma extração real presa em "Em andamento" em `medlog.dalc.in`, com link direto para a tela.

**Defeito 3 — mesma classe do Defeito 1, num `{#each}` diferente.** `ExtractionReview.svelte:442`: `{#each review.unmapped as u, i (u.label + u.collectedAt)}`, na lista de "Pendências do catálogo". O laudo real tem duas linhas pendentes com **rótulo e data idênticos** — Bastonetes aparece como percentual (`%`) e como contagem absoluta (`/mm3`) na mesma coleta, e ambos ficam fora do catálogo. A chave colide, `each_key_duplicate` não tratado aborta o flush de montagem, e a tela trava — desta vez a mensagem falsa é "Em andamento", porque é o último selo que a tela chegou a pintar antes de travar, **não** o estado real: a extração já estava `succeeded` no banco, sem erro, havia minutos. Corrigido trocando a chave para o índice `i`, que o resto do bloco (`promoteCode[i]`, `promoted[i]`, `promoteError[i]`) já tratava como identidade do item — o `{#each}` estava dessincronizado do próprio estado do formulário que ele desenha. Commit `8ff60f3`.

**Varredura preventiva:** `grep` em todo `{#each ... as ... (` do frontend depois do fix. Todos os outros 20+ blocos já chaveiam por `.id`/`.code`/`.model`/chave de `Map` — únicos de verdade. Só os dois desta sessão (Defeito 1 e Defeito 3) usavam campo de exibição como chave; não há um quarto candidato pendente.

**Verificado ao vivo, não só lido:** build local, extração semeada com as duas pendências "Bastonetes" colididas (mesmo rótulo, mesma data, unidades diferentes). Antes da correção: `pageerror: each_key_duplicate`, tela presa. Depois: as duas pendências renderizam distintas, com formulário de promoção próprio para cada uma.

**Achado que muda o hábito de verificação em produção, não só um bug de código:** ao investigar, o bundle servido em `medlog.dalc.in` era `index-DPyFC8YA.js` — o build de **antes de qualquer um dos três fixes de hoje**, Defeito 1 e Defeito 2 inclusive. O GitHub Actions publica `ghcr.io/edalcin/medlog:latest` sozinho a cada push em `main` (`.github/workflows/publish-docker.yml`), mas **isso não atualiza o container do Unraid** — falta o `docker pull` + recriação, que é ação humana (Unraid → Docker → Check for Updates → Update). Presumir que um commit no GitHub já está no ar é o erro: a instância pode estar rodando qualquer coisa entre o último `docker pull` manual e o `HEAD` do repositório. `UNRAID.md` ganhou o aviso na seção de Atualização.

**Instrução do usuário, respeitada à risca:** acesso à base de produção via SSH (`192.168.1.10`, chave dedicada só para isso) só para diagnóstico somente-leitura, com backup antes de qualquer consulta, e **nenhuma alteração manual no container**. Os três commits de hoje estão em `main`, publicados no GHCR pelo CI; o `docker pull` + recriação em produção fica para o usuário decidir quando.
...
- Esse mesmo defeito de chave de `{#each}` **já apareceu duas vezes** neste projeto (`HealthSeries.svelte` e `ExtractionReview.svelte`), sempre com a mesma forma: combinar dois campos de texto que o laudo real frequentemente repete (mesmo texto+fonte de faixa; mesmo rótulo+data de analito pendente). Ao tocar um `{#each}` novo com chave composta de campos de exibição, prefira o índice `i` ou um campo que já seja identidade reconhecida no resto do componente (`promoteCode[i]` já indexava por posição — a chave do `{#each}` devia ter seguido o mesmo índice desde o início).
- Selo de estado na tela pode mentir depois de um crash de render: se um `each_key_duplicate` (ou qualquer erro não tratado) acontece durante o flush que troca o selo de "Em andamento" para o estado final, a tela fica presa mostrando o selo **anterior** ao crash, não o estado real. Confira sempre o banco/API antes de confiar num "travado" — muitas vezes já terminou.
- CI publicar a imagem no GHCR a cada push em `main` **não** significa que a produção está atualizada. `publish-docker.yml` builda e publica sozinho; o `docker pull` + recriação do container é sempre ação humana, aqui deliberadamente fora do alcance do agente (instrução do usuário: nenhuma alteração de Docker em produção). Ao investigar um bug relatado em produção, confira o hash do bundle servido (ou o horário de start do container) antes de presumir que o `HEAD` do repositório é o que está no ar.
