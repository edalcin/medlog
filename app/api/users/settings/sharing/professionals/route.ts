...
export async function PUT(request: Request) {
  try {
    ...
    await prisma.userProfessionalSharing.deleteMany({
      where: {
    ...
    if (validatedData.userIds.length > 0) {
      await prisma.userProfessionalSharing.createMany({
    ...
    const updatedSharingList = await prisma.userProfessionalSharing.findMany({
      where: {
        sharingFromUserId: session.user.id,
      },
      select: {
    ...
    return successResponse(
      ...
      'Compartilhamento de profissionais atualizado com sucesso'
    )
  } catch (error) {
    ...
    console.error('Error updating professionals sharing:', error)
    return errorResponse('Erro ao atualizar compartilhamento de profissionais', 500)
  }
}
...
export async function GET(request: Request) {
  try {
    ...
    const sharingList = await prisma.userProfessionalSharing.findMany({
      where: {
        sharingFromUserId: session.user.id,
      },
      select: {
    ...
    console.error('Error fetching professionals sharing:', error)
    return errorResponse('Erro ao buscar compartilhamento de profissionais', 500)
  }
}
